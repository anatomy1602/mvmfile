...
做 QQ 机器人项目时，我遇到了一个反复出现的问题：

**内容写好了，但要适配不同的呈现场景时，就得重写一遍。**
...
这和 PPT 的痛点一模一样：**内容写进去的那一刻，呈现方式就被锁死了。**

Markdown 解决了一部分问题（纯文本、可 diff、Git 友好），但它的结构能力太弱——`#` `**` `>` 这些符号只能表达层级和强调，无法精确描述「这是一个测验」「这是一个时间线」「这是一个闪卡」。

所以我想要一种格式：
1. 编辑体验像 Markdown 一样自然
2. 有精确的类型系统，程序可以解析
3. 内容和呈现完全分离
4. 同一份内容可以渲染成任意形式
